...
//   //       const apiUrl = role == 1 ? `${process.env.REACT_APP_SUPER_ADMIN_API}/geofences` : role == 2 ? `${process.env.REACT_APP_SCHOOL_API}/geofences` : `${process.env.REACT_APP_BRANCH_API}/geofences`
...
//     try {
//       const token = localStorage.getItem("token");
//       const apiUrl = role == 1 ? `${process.env.REACT_APP_SUPER_ADMIN_API}/geofences` 
...
//       // Iterate over each deviceId and extract geofence data
//       const allDeviceData = Object.keys(response.data).flatMap(deviceId => {
//         const geofenceData = response.data[deviceId];
//         return geofenceData.map(item => ({
//           ...item, 
//           deviceId, // Include deviceId in each row
//         }));
//       });
  
//       // Apply local date filtering if dates are provided
//       const filteredData = (startDate || endDate) 
//         ? allDeviceData.filter(row => {
//             const registrationDate = parseDate(row.formattedRegistrationDate); // Assuming there's a date field in geofence data
//             const start = parseDate(startDate);
//             const end = parseDate(endDate);
//             return (!startDate || registrationDate >= start) && 
//                    (!endDate || registrationDate <= end);
//           })
//         : allDeviceData; // If no date range, use all data
...
//         reversedData.map(row => ({ ...row, isSelected: false }))
//       );
//       setOriginalRows(allDeviceData.map(row => ({ ...row, isSelected: false })));
//       setTotalResponses(reversedData.length);
...
//           if (!response.ok) {
//             throw new Error(
//               `Error deleting record with ID ${id}: ${response.GeofenceText}`
//             );
//           }
...
//       // Check if the response is okay (Geofence code 200-299)
//       if (!response.ok) {
//         throw new Error(`HTTP error! Geofence: ${response.Geofence}`);
//       }
...
//   //     const token =
//   //       "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY2YjRhMDdmMGRkYmVjNmM3YmMzZDUzZiIsInVzZXJuYW1lIjoiYWRtaW4iLCJpYXQiOjE3MjMxMTU1MjJ9.4DgAJH_zmaoanOy4gHB87elbUMod8PunDL2qzpfPXj0"; // Replace with actual token
//   //     const apiUrl = `https://schoolmanagement-6-6tcs.onrender.com/school/Geofence/${id}`;
...
//   return (
//     <>
//       <h1 style={{ textAlign: "center", marginTop: "80px" }}>Geofence</h1>
//       <div>
//         <div
//           style={{
//             display: "flex",
...
//                               <TableCell style={{ borderRight: "1px solid #e0e0e0", fontWeight: "bold" }}>Pickup Geofence</TableCell>
//                               <TableCell style={{ borderRight: "1px solid #e0e0e0", fontWeight: "bold" }}>Drop Geofence</TableCell>
...
//                               <TableCell style={{ borderRight: "1px solid #e0e0e0" }}>{detailedData[row._id].pickupGeofence}</TableCell>
//                               <TableCell style={{ borderRight: "1px solid #e0e0e0" }}>{detailedData[row._id].dropGeofence}</TableCell>
...
//             <td>{detailedData[row._id].pickupGeofence}</td>
//             <td>{detailedData[row._id].dropGeofence}</td>
