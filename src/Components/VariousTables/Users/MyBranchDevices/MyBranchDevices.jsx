...
export const MyBranchDevices = () => {
  ...
  const fetchData = async (startDate = "", endDate = "") => {
    ...
    try {
      ...
      const schoolData = response.data; // Assuming this contains schoolName, branchName, and devices
  
      // Check if devices is an array
      if (Array.isArray(schoolData.devices)) {
        const devices = schoolData.devices;
  
        // Apply local date filtering if dates are provided
        const filteredData =
          startDate || endDate
            ? devices.filter((device) => {
                const registrationDate = parseDate(device.formattedRegistrationDate);
                const start = parseDate(startDate);
                const end = parseDate(endDate);
            ...
            : devices; // If no date range, use all devices
  
        const reversedData = filteredData.reverse();
  
        // Log the date range and filtered devices
        console.log(`Devices fetched between ${startDate} and ${endDate}:`);
        ...
        console.error("Expected an array but got:", schoolData.devices);
      }
    } catch (error) {
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(workbook, "MyBranchDevices.xlsx");
  };
